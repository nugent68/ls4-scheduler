...
{
    double ra,dec,ra_rate,dec_rate,ha;

    if (argc!=3){
        fprintf(stderr,"syntax: test_rate ha dec\n");
        exit(-1);
    }
    ...
    fprintf(stdout," ha = %12.6f, dec = %12.6\n",ha,dec);
    fprintf(stdout," ra rate = %12.6f, dec rate = %12.6f\n",
         get_ra_rate(ha,dec),get_dec_rate(ha,dec));
...
/* ha is in hours, dec in degrees. Return offset to RA tracking rate
   (to be added to siderial rate) to correct tracking error. Units are
    arcsec/hour */
